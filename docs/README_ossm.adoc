...
+
-----
$ sed "s|%API_MGMT_BACKEND_HOST%|$API_MGMT_BACKEND_HOST|" ./ossm/apimgmt/serviceentry-backendURL.yml |  oc apply -n user1-services -f -
-----
+
NOTE:  Without this ServiceEntry, an error should appear in the _envoy-proxy_ logs similar to the following: 
+
-----
...
. Set the log level of the _wasm_ module of _istio-proxy_ to debug: 
+
-----
