...
export OCP_GWAPI_DOMAIN=gwapi$(oc whoami --show-console | awk 'BEGIN{FS="apps"}{print $2}')
...
export BACKEND_GWAPI=http://backend-oidc.$OCP_GWAPI_DOMAIN
...
-----
$ curl -v -H "Accept: application/json" \
        -X GET $BACKEND_GWAPI/unsecured/sanityCheck | jq -r .
-----
