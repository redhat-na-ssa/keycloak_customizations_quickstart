...
. New tenant creation; provider_key from {{ tenant_name }}-generated-secret is used
...
. New tenant creation; provider_key from {{ tenant_name }}-generated-secret is used to create new access token for use when creating backend and product:
