...
-----
$ RPT=$(curl -X POST "$ACCESS_TOKEN_URL" \
            --user "bservice:password" \
            -d "grant_type=urn:ietf:params:oauth:grant-type:uma-ticket" \
            -d "audience=bservice" \
            | sed 's/.*access_token":"//g' | sed 's/".*//g')
-----

. View _Permissions_ associated with _bservice_ protected resource:
+
-----
...
. View _realm_access_ roles associated with the _bservice_ protected resource:
+
-----
...
[{"name":"Default Resource","type":"urn:bservice:resources:default","owner":{"id":"2dde291e-8de5-4dc1-8163-2477931bbeee","name":"bservice"},"ownerManagedAccess":false,"attributes":{},"_id":"ed1b6721-79f7-4f19-9216-e55590748df8","uris":["/*"]}]
-----
