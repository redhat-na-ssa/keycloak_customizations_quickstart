...
+
-----
RHSSO_HOST=sso.local
RHSSO_URL=http://$RHSSO_HOST:4080
REALM_ID=ldap-demo
SSO_CLIENT_ID=ldap-app
ACCESS_TOKEN_URL="$RHSSO_URL/realms/$REALM_ID/protocol/openid-connect/token"
BACKEND_ROUTE=http://localhost:6080
...
. Start all linux containers for this demo:
+
-----
...
+
image::docs/images/browser_network_tools.png[]
...
The User Federation configs found in the demo specify a _READ_ONLY_ strategy for synchronizing federated user data between RH-SSO and LDAP.  
...
In this section of the lab, you will modify the synchronization strategy to allow changes of users in RH-SSO to propogate to LDAP.


. In the _ldap-apacheds_ User Federation config, switch the _Edit Mode_ to:  `WRITEABLE`.
...
ACCESS_TOKEN_URL="$RHSSO_URL/auth/realms/$REALM_ID/protocol/openid-connect/token"
...
All of the same tests conducted in your local environment can be executed in your OpenShift environment.
...
=== Modify User Federation

Your SSO Realm includes a pre-configured _User Federation_ called:  `ldap-apacheds`.
This User Federation will need to be modified such that it prevents writing to the LDAP provider when a new user is sync'ed from an external OAuth Identity Provider.

. Navigate to: `User Federation -> ldap-apacheds`
. Change the value of `Edit Mode` to:  _READ_ONLY_
+
image::docs/images/modify_ldap-apacheds.png[]
. Click `Save`
