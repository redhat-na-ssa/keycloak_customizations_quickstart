...
. Retrieve an OAuth2 _access token_ from the SSO client :
+
-----
...
-----
+
NOTE:  By setting a `scope=openid`, the OIDC _id_token_ is also included in the authentication response.
...
The User Federation configs found in the demo specify a _READ_ONLY_ strategy for synchronizing federated user data between RH-SSO and LDAP.  Details regarding the various synchronization strategies supported in RH-SSO can be found in the section _Integrating with LDAP and Active Directory_ of link:https://smile.amazon.com/Keycloak-Management-Applications-protocols-applications/dp/1800562497[Keycloak - Identity & Access Management for Modern Apps] book.
...
=== Smoke Test
. Retrieve an OAuth2 _access token_  :
+
-----
TKN=$(curl -X POST "$retrieve_token_url" \
            -H "Content-Type: application/x-www-form-urlencoded" \
...
. By setting _fullScopeAllowed=true_ in SSO client, all roles assocated with an authenticated user will be included in the _access token_.
+
These roles can be visualized as follows:
+
-----
